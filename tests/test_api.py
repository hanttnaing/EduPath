...
# ---------------------------------------------------------
# Scholarship recommendation history tests
# ---------------------------------------------------------

SCHOLARSHIP_HISTORY_TEST_USER_ID = (
    "user_api_scholarship_history_test_001"
)


def delete_scholarship_history_test_user() -> None:
    """Remove the temporary scholarship-history test user."""

    database = get_database()

    database["user_profiles"].delete_many(
        {
            "user_id": SCHOLARSHIP_HISTORY_TEST_USER_ID
        }
    )


def build_scholarship_history_test_profile(
    preferred_country: str = "Japan",
) -> dict:
    """Return valid data for the temporary test profile."""
...
def create_scholarship_history_test_user(
    client: TestClient,
    preferred_country: str = "Japan",
) -> None:
    """Create a fresh scholarship-history test profile."""

    delete_scholarship_history_test_user()

    response = client.post(
        "/api/user-profiles",
        json=build_scholarship_history_test_profile(
            preferred_country=preferred_country,
        ),
    )
...
def get_scholarship_history_test_endpoint(
    top_k: int = 5,
) -> str:
    """Return the scholarship recommendation endpoint."""

    return (
        f"/api/recommendations/scholarships/"
        f"{SCHOLARSHIP_HISTORY_TEST_USER_ID}"
        f"?top_k={top_k}"
    )


def get_scholarship_history_read_endpoint() -> str:
    """Return the recommendation-history endpoint."""
...
def test_scholarship_recommendation_history_is_created(
    client: TestClient,
) -> None:
    """One scholarship run should create one history item."""

    create_scholarship_history_test_user(client)

    try:
        recommendation_response = client.get(
            get_scholarship_history_test_endpoint()
        )
        ...
        expected_scholarship_ids = [
            recommendation["scholarship_id"]
            for recommendation in recommendations
        ]

        history_response = client.get(
            get_scholarship_history_read_endpoint()
        )
...
def test_scholarship_recommendation_history_persists_in_profile(
    client: TestClient,
) -> None:
    """The history item should persist in the profile."""
    ...
    try:
        ...
        # Saving history must not affect saved opportunities.
        assert profile["saved_universities"] == []
        assert profile["saved_scholarships"] == []
...
def test_scholarship_recommendation_history_appends_runs(
    client: TestClient,
) -> None:
    """Two recommendation runs should create two records."""
...
def test_scholarship_recommendation_history_saves_empty_result(
    client: TestClient,
) -> None:
    """A zero-result scholarship run should be recorded."""

    create_scholarship_history_test_user(
        client,
        preferred_country="Singapore",
    )
...
def test_scholarship_recommendation_history_not_saved_for_invalid_top_k(
    client: TestClient,
) -> None:
    """An invalid request must not create history."""
